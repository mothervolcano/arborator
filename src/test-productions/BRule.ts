...
class BRule extends Production {
	...
	public process(params?: string) {
		...
		if (this.head.symbol === "K" && params) {
			...
			stream.push(
				...[
					{ glyph: debugMark, id: 99, data: {} },
					{ glyph: debugInfo, id: 99, data: {} },
				],
			);
			...
			if (metaGlyph.glyph.type === "Rule") {
				...
				if (metaGlyph.data.prims && metaGlyph.data.prims.length) {
					...
					for (const primUpdate of metaGlyph.data.prims) {
						const prim = metaGlyph.glyph.prims.find(
							(p) => p.prefix === primUpdate.prefix,
						);
						...
						if (prim) {
							...
							throw new Error(
								`Processing ERROR @ ${this.head.symbol} RULE: ${primUpdate.prefix} Prim not found on ${metaGlyph.glyph.symbol}. `,
							);
						}
					}
