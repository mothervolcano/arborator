...
import {
	...
	IncompleteParameterStringException,
...
class Composer implements IComposer {
	...
	private extractParameters(str: string) {
		try {
			...
			if (error instanceof IncompleteParameterStringException) {
				...
				// Unhandled error
				throw new Error(
					`! Failed to parse parameters in string: ${str}`,
				);
			}
		}
	}
	...
	public compose(iterations: number, context?: any): string {
		...
		do {
			...
			if (openMarkerIdx !== -1 && closeMarkerIdx !== -1) {
				const params = this.commands
					.splice(openMarkerIdx, closeMarkerIdx - openMarkerIdx + 1)
					.join("");
	...
	public plot() {
		...
		for (let i = 0; i < this.commands.length; i++) {
			...
			const nextSymbol =
				this.commands[Math.min(i + 1, this.commands.length)];
			...
			if (typeof currSymbol === "string") {
				...
				switch (glyph.type) {
					case "Rule":
						command = this.model.hasCommand(glyph.symbol)
							? this.model.getCommand(glyph.symbol)
							: undefined;
					...
					default:
						throw new Error(
							`Failed to execute. Probably invalid Glyph ${currSymbol}`,
						);
				}
			}
		}
