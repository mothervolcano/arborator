...
class Alphabet implements IAlphabet {
	...
	public registerGlyph(type: GlyphType, entry: string) {
		// entry can be a single character eg. 'A' or a series of characters to be split into separate entries 'A+FB'
		...
		for (const char of entry) {
			...
			switch (type) {
				...
				case "Instruction":
					glyph = { id: 0, type: type, symbol: symbol };
				...
				default:
					throw new Error(`${type} is an invalide Glyph Type`);
			}
	...
	public glyph(symbol: string): Glyph {
		if (symbol.length === 1) {
			const _glyph = this.dictionary.get(
				this.validateRetrieval(symbol.charAt(0)),
			)!;

			return _glyph;
		} else if (symbol.length > 1) {
			throw new Error(
				`Invalid input ${symbol}. If you meant to retrieve a series of Glyphs use composePhrase instead`,
			);
		} else {
	...
	public rule(symbol: string): Rule {
		if (symbol.length === 1) {
			...
			if (_glyph.type === "Rule") {
				...
				throw new Error(
					`Requested symbol ${symbol} is not a Glyph Rule`,
				);
			}
		} else if (symbol.length > 1) {
			throw new Error(
	...
	public collect(symbols: string): Glyph[] {
		if (symbols.length > 1) {
			...
			if (sequence.length > 0) {
				...
				throw new Error(`Failed to retrieve entry(ies) for ${symbols}`);
			}
		} else if (symbols.length === 1) {
			throw new Error(
				`Invalid input ${symbols}. If you mean to retrieve a single Glyph use retrieveGlyph instead`,
			);
		} else {
